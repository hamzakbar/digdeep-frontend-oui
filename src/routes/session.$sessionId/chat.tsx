          ...
          <section className='space-y-4'>
            ...
            <div className='space-y-2'>
              {files?.map((file) => (
                <div
                  key={file.name}
                  className='flex items-center gap-3 p-3 rounded-2xl bg-white border border-border/40 shadow-sm hover:border-primary/30 transition-all cursor-pointer group'
                >
                  <div className='size-8 rounded-xl bg-primary/5 flex items-center justify-center'>
                    <FileText className='size-4 text-primary opacity-60' />
                  </div>
                  <div className='flex-1 overflow-hidden'>
                    <p className='text-xs font-bold truncate text-foreground/70'>
                      {file.name}
                    </p>
